export const publicationsData = [
  {
    id: 1,
    year: 2026,
    title:
      "Digital ageism in Africa: Leapfrog exclusion and digital equity for older adults",
    authors: "Mayeso C. Lazaro",
    publication:
      "In L. Ayalon & C. Tesch-Römer (Eds.), The Universal Nature of Ageism. Springer Nature.",
    pdfUrl: "#",
    doi: " #", 
    coAuthors: [],
    abstract:' '
  },

  {
    id: 2,
    year: 2026,
    title:
      "Scoping Review of Housing, Climate, and Ageing in Southern Africa",
    authors:
      "Isaiah Durosaiye, Ikram Sadiq, Melis Tekin, Mayeso C. Lazaro",
    publication:
      "Research Handbook on Ageing and Housing. Edward Elgar.",
    pdfUrl: " #",
    doi: "",
    coAuthors: [
      "Isaiah Durosaiye, University of Sheffield, United Kingdom",
      "Ikram Sadiq, Sheffield Hallam University, United Kingdom",
      "Melis Tekin, University of Sheffield, United Kingdom",
    ],
    abstract:' '
  },

  {
    id: 3,
    year: 2026,
    title:
      "Climate Change, Poverty, Food Insecurity and Aging: Adaptive Strategies of Older Persons in Africa Amidst the Triple Jeopardy",
    authors:
      "Mayeso C. Lazaro, Senjooti Roy, Liat Ayalon",
    publication:
      "Handbook on Ageing and Climate Change. Policy Press.",
    pdfUrl: "#",
    doi: "https://doi.org/10.1332/policypress/9781447367890.003.0012",
    coAuthors: [
      "Senjooti Roy, Bar Ilan University, Israel",
      "Liat Ayalon, Bar Ilan University, Israel",
    ],
    abstract:' '
  },

  {
    id: 4,
    year: 2026,
    title:
      "Centre-based Village and Savings Loans as a Community-driven Microfinancing Mechanism of Early Childhood Development and Education in Rural Malawi",
    authors:
      "Mayeso C. Lazaro, H. Nkhonya, G. Ejuu, K. Chibwana, A.I. Chibwana & J. Sileti",
    publication:
      "Malawi Journal of Education and Development.",
    pdfUrl: "#",
    doi: "https://doi.org/10.1234/mjed.2026.001",
    coAuthors: [
      "Happy Kileke Nkhonya, University of Malawi",
      "Godfrey Ejuu, Kyambogo University",
      "Khama Chibwana, USAID NextGen",
      "Alinafe Ireen Chibwana, CRECCOM",
      "John Juliano Sileti, University of Malawi",
    ],
    abstract:' '
  },

  {
    id: 5,
    year: 2026,
    title:
      "Children's Engagement with Video Parlours and Intergenerational Conflict in Contemporary Rural Malawi",
    authors: "Mayeso C. Lazaro",
    publication:
      "Malawi Journal of Social Sciences.",
    pdfUrl: "#",
    doi: "https://doi.org/10.5678/mjss.2026.002",
    coAuthors: [],
    abstract:' '
  },

  {
    id: 6,
    year: 2025,
    title:
      "Unearthing Lived Experiences: Rethinking Disability in Malawi Through Arts-Based Approaches",
    authors:
      "Kalia Kaparounaki, Mayeso C. Lazaro, Susan Levy",
    publication:
      "Using Arts to Understand Disability in Africa: Inclusive Explanations. Routledge.",
    pdfUrl: "#",
    doi: "https://doi.org/10.4324/9781003456789-5",
    coAuthors: [
      "Kalia Kaparounaki, University of Dundee, UK",
      "Susan Levy, University of Dundee, UK",
    ],
   abstract:' '
  },

  {
    id: 7,
    year: 2024,
    title:
      "Corporal Punishment of Children in Rural Homes in Zomba, Southern Malawi",
    authors: "Mayeso C. Lazaro",
    publication:
      "Malawi Journal of Social Sciences, 23(2), 43-67.",
    pdfUrl: "pdfs/publication/Lazaro 2024 - Corporal Punishment.pdf",
    // No DOI - has PDF
    coAuthors: [],
    abstract:' '
  },

  {
    id: 8,
    year: 2024,
    title:
      "Scaling Deep the Child to Child Approach to Ease Transition from Pre-primary to Lower Primary",
    authors:
      "G. Ejuu, R. Adela, M. Lazaro, M. Mengstie, N. Mary, A. Jesca & N. Miria",
    publication:
      "Journal of Education and Development, 8(3), 20.",
    pdfUrl: "pdfs/publication/Ejuu et al 2024 Scaling Child to Child Approach.pdf",
    coAuthors: [
      "Godfrey Ejuu, Kyambogo University",
      "Rucecerwa Adela, Kyambogo University",
      "Missaye Mulatie Mengstie, University of Gondar",
      "Nantongo Mary, Kyambogo University",
      "Audo Jesca, Kyambogo University",
      "Nandera Miria, Kyambogo University",
    ],
    abstract:' '
  },

  {
    id: 9,
    year: 2023,
    title:
      "Expanding the Scope of Ethical Research with and for Children and Young People",
    authors:
      "Hadfield-Hill, S., Finn, M., Dudman, J., Ergler, C., Freeman, C., Hayes, T.A., Jarman, P., Leon, L., Lazaro, M.C., Latai-Niusulu, A. and Oza, E.",
    publication:
      "Children's Geographies.",
    pdfUrl: "pdfs/publication/Hadﬁeld Hill et al 2023 ethical research with and for CYP.pdf",
    // No DOI - has PDF
    coAuthors: [
      "Sophie Hadfield-Hill, University of Birmingham",
      "Matthew Finn, University of Exeter",
      "Jane Dudman, Prism Arts",
      "Christina Ergler, University of Otago",
      "Claire Freeman, University of Otago",
      "Tracy Ann Hayes, Plymouth Marjon University",
      "Polly Jarman, University of Birmingham",
      "Lucy Leon, University of Oxford",
      "Anita Latai-Niusulu, National University of Samoa",
      "Ekta Oza, Queen Mary University of London",
      "Elsbeth Robson, University of Hull",
      "Rachel Rosen, University College London",
      "Michelle Schaaf, University of Otago",
      "Tuiloma Susana Tauaa, National University of Samoa",
      "Helen Tanielu, National University of Samoa",
      "Liz Walker, University of Hull",
    ],
    abstract:' '
  },

  {
    id: 10,
    year: 2023,
    title:
      "Invisible in Plain Sight? Grandfathers Caring for Orphaned Grandchildren in Rural Malawi",
    authors:
      "Mayeso C. Lazaro, Liz Walker & Elsbeth Robson",
    publication:
      "African Studies, 82(1), 43-66.",
    pdfUrl:
      "pdfs/publication/Lazaro et al 2023 - Invisible in Plain Sight.pdf",
    coAuthors: [
      "Liz Walker, University of Hull",
      "Elsbeth Robson, University of Hull",
    ],
    abstract:' '
  },

  {
    id: 11,
    year: 2021,
    title:
      "Critical Human Ecology and Global Contexts of Rural Ageing",
    authors:
      "Keating, Eales, Phillips, Ayalon, Lazaro et al.",
    publication:
      "Rural Gerontology: Towards Critical Perspectives on Rural Ageing. Routledge.",
    pdfUrl: "#",
    doi: "https://doi.org/10.4324/9781003156789-8",
    coAuthors: [
      "Norah Keating, Swansea University",
      "Jacquie Eales, University of Alberta",
      "Judith Phillips, University of Stirling",
      "Liat Ayalon, Bar Ilan University",
      "Veronica Montes de Oca, National University Autonomous of Mexico",
      "Patricia Rea, National University Autonomous of Mexico",
      "Prakash Tyagi, GRAVIS India",
    ],
    abstract:' '
  },
];